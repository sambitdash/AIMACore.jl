...
replace{S<:State}(pq::PriorityQueue{S,Node{S}}, node::Node{S})= (pq[node.state] = node)
...
"""
failure(node) = error("Node could not be reached.")
...
pg. 82 Fig. 3.11 AIMA 3ed
"""
mutable struct UniformCostSearch{PQ, SET} <: SearchAlgorithm
    frontier::PQ
    explored::SET

    UniformCostSearch{PQ, SET}() where {PQ, SET} = new(PQ(),SET())
end
    ...
    while(true)
        ...
        for action in actions(problem, node.state)
            ...
            elseif (child.state in search.frontier) &&
                    (child.path_cost < get(search.frontier, child.state, Node(problem.initial_state)).path_cost)
